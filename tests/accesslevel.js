...
Test('/accesslevel', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         * responses: 200
         */
        t.test('test accesslevel_create post operation', function (t) {
            Mockgen().requests({
                path: '/accesslevel',
                operation: 'post'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/accesslevel']['post']['responses']['200']['schema']);
         ...
         * responses: 200
         */
        t.test('test accesslevel_update put operation', function (t) {
            Mockgen().requests({
                path: '/accesslevel',
                operation: 'put'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/accesslevel']['put']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
