...
Test('/namecard', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         * responses: 200
         */
        t.test('test namecard_getByCurrentUserId get operation', function (t) {
            Mockgen().requests({
                path: '/namecard',
                operation: 'get'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/namecard']['get']['responses']['200']['schema']);
         ...
         * responses: 200
         */
        t.test('test namecard_create post operation', function (t) {
            Mockgen().requests({
                path: '/namecard',
                operation: 'post'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/namecard']['post']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
         ...
         * responses: 200
         */
        t.test('test namecard_update put operation', function (t) {
            Mockgen().requests({
                path: '/namecard',
                operation: 'put'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/namecard']['put']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
