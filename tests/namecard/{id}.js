...
/**
 * Test for /namecard/{id}
 */
Test('/namecard/{id}', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         * responses: 200
         */
        t.test('test namecard_getById get operation', function (t) {
            Mockgen().requests({
                path: '/namecard/{id}',
                operation: 'get'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/namecard/{id}']['get']['responses']['200']['schema']);
         ...
         * responses: 200
         */
        t.test('test namecard_deleteById delete operation', function (t) {
            Mockgen().requests({
                path: '/namecard/{id}',
                operation: 'delete'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/namecard/{id}']['delete']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
